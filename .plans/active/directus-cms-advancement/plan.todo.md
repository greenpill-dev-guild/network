...
- [ ] Align remaining self-publish copy with shipped behavior: role
...
- [ ] Production re-apply after the 2026-08-10 image-upload shape change:
...
- [ ] Add a Directus/admin row to `docs/agentic-mcp-tooling-runbook.md`
      (primary use, do-not-use, proof surface = steward smoke + content-access
      tests) so admin changes have a declared proof lane.
- [ ] Rename `content:snapshot` or its description so nobody reads it as
...
- [ ] Dispatch-on-publish: agent-side watcher (durable queue like
...
- [ ] Content-review notifications: `pending_review` update requests and
...
- [ ] Review-outcome visibility: widen scoped steward read filter on
...
- [ ] Migration: trigger to default `published_at`/`reviewed_at` (and
...
- [ ] Impact loop: schedule `impact:sync` as an agent in-process sweep (same
...
- [ ] Defuse all-or-nothing snapshot asserts: per-record quarantine (drop the
...
- [ ] Replace per-slug scoped policies with one static `Greenpill Assigned
...
- [ ] Revocation + cleanup: migrate existing stewards to the dynamic policy,
...
- [ ] Chapters/guilds form structure: field groups (Identity / Story / Links
      & Media / Impact / SEO / Workflow), workflow fields grouped last,
      `slug` visually separated with its warning note.
- [ ] Structured editors: replace raw JSON `input-code` for `links`,
...
- [ ] Studio metadata for the raw intake collections operators do see
