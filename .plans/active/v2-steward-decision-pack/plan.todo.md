# V2 Steward Decision Pack Plan

## Sequencing

- [x] Confirm the relevant repo baseline and current docs
- [x] Create the active `.plans` hub for steward decisions
- [x] Refine scope and defaults in `spec.md`
- [x] Link the steward-facing docs from this hub
- [ ] Share `docs/v2/steward-brief-one-page.md` with stewards
- [ ] Share `docs/v2/steward-telegram-polls.md` in Telegram
- [ ] Collect poll results and nomination replies
- [ ] Summarize responses in `reports/steward-response-summary.md`
- [ ] Update canonical V2 docs if the responses change narrative or first-release priorities
- [ ] Update `status.json` as decisions or readiness change
- [x] Capture the handoff or follow-up note in `handoffs/README.md`
- [x] Run `node scripts/plan-hub.mjs validate`

## Decision Outputs

- [ ] chosen homepage promise
- [ ] chosen homepage proof model
- [ ] Greenpill Garden first-release framing
- [ ] first featured chapters
- [ ] first featured guilds or pods
- [ ] first featured builder projects or protocols
