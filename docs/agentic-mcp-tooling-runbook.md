...
- The self-hosted Directus admin runs its built-in MCP server (enabled by `bun run directus:studio:setup`; `mcp_allow_deletes` stays off). Agent access goes through the API-only `Greenpill Content Agent` role - read operational content, propose chapter update requests, never publish, never touch intake/users/files - with per-user static tokens minted in the Directus UI and never committed. Client configuration stays local/session-specific: no project-scoped `.mcp.json` is committed to this repo, and connecting a client is an explicit local step, not repo config.
...
| `packages/admin` Directus + `bun run directus:*` scripts | Authenticated steward CMS: roles/policies/permissions, Data Studio metadata, scoped chapter/guild access, steward onboarding, map moderation queue. | A public API surface, route-local privacy filtering, Greenpill schema migrations (those stay in `packages/agent/migrations`), or storing automation secrets. | `bun run directus:steward:smoke`; `bun run directus:map-moderation:smoke`; `bun test scripts/directus-content-access.test.ts scripts/directus-studio-setup.test.ts scripts/directus-users.test.ts`; local rehearsal via `bun run db:migrate` + `bun run directus:local:bootstrap`. Any change to roles/policies/permission shapes must re-run setup + `directus:content-access -- sync` wherever it is already applied. |
