# Greenpill Graph Data

Core graph files:

- [greenpill-network.graph.json](greenpill-network.graph.json)
  - original network graph
- [greenpill-network-expanded.graph.json](greenpill-network-expanded.graph.json)
  - expanded graph with the podcast overlay merged in

Podcast-specific graph files:

- [podcast/README.md](podcast/README.md)

These files are structured for easy loading into browser-based graph explorers.

It separates:

- `relationships`
  - structural connections
- `capital_flows`
  - directional value exchange

Schema notes:

- [greenpill-graph-schema.md](../../docs/research/greenpill-graph-schema.md)
