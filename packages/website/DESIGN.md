...
- **Viewport `@media` is not the reflow mechanism.** `ui:check` flags viewport `@media` in `packages/website/src`; introduce one only as a documented, reviewed exception.
- **Everything inside `#main-content` reflows via `@container`.** `#main-content` (in `GpLayout`) is the page-level query container, so page sections, primitives, and in-content components (incl. `Breadcrumb`) reflow against it without setting their own `container-type`. A component that needs to reflow on its *own* width (e.g. a card reused across columns) sets `container-type` on its root.
- **Sanctioned non-`@container` responsive cases (do not flag these):** (1) the global type scale — fluid `clamp()`; (2) touch-target / input-modality sizing — `@media (pointer: coarse)`, since tap size depends on the device, not container width; (3) top-level full-viewport chrome — the **site header and footer** are siblings of `#main-content` and always span the viewport (the header is `position: sticky`), so they use viewport `@media` rather than forcing `container-type` onto a sticky element. Everything else reflows with `@container`.

### Fluid typography — `clamp()`
- Display and headline sizes are **`clamp(min, preferred, max)`**, derived from the established mobile→desktop values (e.g. display `42px → 72px`). Type scales smoothly with the viewport — no breakpoint jumps, no per-breakpoint type tokens.
- Body, label, overline, and caption sizes stay **fixed** (they did not scale before and should not now).
- Sizes live in `src/styles/gp-tokens.css`; never hardcode font sizes in components.

### Mobile heights — dynamic viewport units
- Use **`dvh` / `svh` / `lvh`**, never `vh`, for full-height, sticky, and overlay layouts, so content is never cut off behind mobile browser chrome. Default to `dvh`; use `svh` when the smallest stable height is required.

### State in CSS — `:has()`
- Prefer **`:has()`** for parent/sibling state (a form containing an error, an active filter, an open menu/disclosure) over toggling classes in JavaScript, where practical. State stays declarative; less script ships.

### Specificity & internationalization
- Global CSS is organized with **cascade layers** (gp-prefixed to avoid colliding with Tailwind's reserved layer names): `@layer gp-reset, gp-tokens, gp-base, gp-components, gp-utilities;`. This stops token/base styles from being accidentally overridden and removes the specificity arms race that produced the legacy `global.css`. (Astro component `<style>` blocks stay scoped; layers govern the global stylesheet.)
- Use **logical properties** — `margin-inline`, `padding-inline`, `inset-inline`, `text-align: start/end`, `border-inline-start`, etc. — instead of physical left/right. The site ships translated content; logical properties make every layout RTL/i18n-ready for free. `ui:check` flags physical properties in new code.

### Agent-legible & accessible markup
Agents and assistive tech read the UI through three channels — the rendered **screenshot**, the **HTML**, and the **accessibility tree** — so these are first-class requirements, not extras:
- Use semantic **`<button>`** / **`<a>`** for actions and links; never a `<div>`/`<span>` with a click handler. If a semantic element is genuinely impossible, add the correct `role` **and** `tabindex`.
- Every input has an associated **`<label for>`** (or `aria-label`). No placeholder-as-label (already required under Form Patterns).
- Actionable elements set **`cursor: pointer`** and meet the **≥44×44px** mobile target.
- **Stable layout:** no load-time layout shift — reserve space for images/embeds; `ui:verify` measures CLS. Agents that screenshot get confused by shifting layouts.
- The accessibility tree must be clean: landmark regions (`main`, `navigation`, `contentinfo`) present, every interactive node has an accessible name, no `generic` element standing in for a control.

### Canonical references
When unsure about a CSS or accessibility technique, defer to these rather than improvising:
- web.dev — [Learn CSS](https://web.dev/learn/css), [Learn Accessibility](https://web.dev/learn/accessibility), [Responsive design](https://web.dev/articles/responsive-web-design-basics)
