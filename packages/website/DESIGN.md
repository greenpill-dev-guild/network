...
- **Every reflowing component needs a query-container ancestor.** `#main-content` (in `GpLayout`) is the page-level container. Components rendered *outside* it — the site header and footer are siblings of `#main-content`, not descendants — must set their own `container-type`, or their `@container` rules silently never match.
- **Touch-target / input-modality sizing uses `@media (pointer: coarse)`, not container queries** — tap-target size depends on the input device, not the container's width. Together with the global type scale (`clamp()`), these are the only sanctioned non-`@container` responsive cases.
