...
export function decorateCachedImpactPayload(row, now = new Date()) {
  if (!row) return null;
...
export async function getCachedChapterImpact(sql, chapterSlug, now = new Date()) {
  const [row] = await sql`
    select
...
export async function saveChapterImpactSnapshot(sql, {
  chapterSlug,
  payload,
  ...
  const publicPayload = assertPublicImpactPayload(payload);
