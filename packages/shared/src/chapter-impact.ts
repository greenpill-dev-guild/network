...
export interface ChapterImpactSources {
  impactEnabled: boolean | string | number;
...
const normalizePublicEvent = (event: UnknownRecord, fallbackType: string): PublicImpactEvent => ({
...
const normalizeEvents = (events: unknown, fallbackType: string, limit = 12): PublicImpactEvent[] => asArray(events)
  .map((event) => normalizePublicEvent(event, fallbackType))
  .filter((event) => event.id || event.title || event.url)
  .slice(0, limit);

const sourceHasValue = (sources: Partial<ChapterImpactSources> | null | undefined): boolean => Boolean(
...
export function shouldRenderChapterImpactUi(
  sources?: Partial<ChapterImpactSources> | null,
  uiEnabled = CHAPTER_IMPACT_UI_ENABLED
): boolean {
  return Boolean(uiEnabled) && normalizeBoolean(sources?.impactEnabled) && sourceHasValue(sources);
}

export function buildChapterImpactEndpoint(chapterSlug: string, baseUrl = CHAPTER_IMPACT_AGENT_BASE): string {
...
export function containsPrivateChapterImpactField(value: unknown, seen: Set<object> = new Set()): boolean {
...
export function normalizeImpactSources(sources: Partial<ChapterImpactSources> & UnknownRecord = {}): ChapterImpactSources {
...
export function toPublicImpactSourceBinding(chapter: {
  id: string;
  data: { name: string; impactSources?: Partial<ChapterImpactSources> & UnknownRecord };
}): PublicImpactSourceBinding | null {
...
export function normalizeKarmaImpactSnapshot(snapshot: UnknownRecord | null | undefined = {}): PublicChapterImpactPayload['karma'] {
  const source: UnknownRecord = isRecord(snapshot) ? snapshot : {};
  const project: UnknownRecord = isRecord(source.project)
    ? source.project
    : source;
  ...
  const communityImpact: UnknownRecord | null = isRecord(source.communityImpact)
...
export function normalizeGreenGoodsImpactSnapshot(snapshot: UnknownRecord | null | undefined = {}): PublicChapterImpactPayload['greenGoods'] {
  const source: UnknownRecord = isRecord(snapshot) ? snapshot : {};
  const garden: UnknownRecord = isRecord(source.garden)
    ? source.garden
    : source;
  ...
  const activity: UnknownRecord = isRecord(source.activity) ? source.activity : source;
  const roleCounts = {
  ...
  return {
    garden: {
      address,
      chainId: normalizeInteger(garden.chainId || source.chainId) || 42161,
...
export function toPublicChapterImpactPayload({
  ...
  chapterSlug?: string;
  chapterName?: string;
  sources?: Partial<ChapterImpactSources> & UnknownRecord;
  karma?: UnknownRecord | null;
  greenGoods?: UnknownRecord | null;
  ...
  const proofLinks: PublicChapterImpactPayload['proofLinks'] = [
    ...
    ...asArray(karmaImpact?.milestones)
      ...
      .map((milestone) => ({ label: milestone.title || 'KarmaGAP milestone', url: milestone.url, source: 'karma' as const })),
  ].filter((link): link is PublicChapterImpactPayload['proofLinks'][number] => Boolean(link)).slice(0, 12);
