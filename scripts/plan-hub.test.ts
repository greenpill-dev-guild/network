...
const SCRIPT_PATH = join(REPO_ROOT, "scripts", "plan-hub.ts");
...
function createFixture() {
  ...
  cpSync(SCRIPT_PATH, join(root, "scripts", "plan-hub.ts"));
...
function runPlanHub(root, args) {
  return spawnSync(process.execPath, ["scripts/plan-hub.ts", ...args], {
    cwd: root,
    encoding: "utf8",
  });
}
